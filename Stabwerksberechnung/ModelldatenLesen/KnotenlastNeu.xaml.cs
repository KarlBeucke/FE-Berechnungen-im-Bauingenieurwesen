...
public partial class KnotenlastNeu
{
    ...
    private void BtnDialogOk_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (_modell.Lasten.TryGetValue(knotenlastId, out var vorhandeneKnotenlast))
        {
            ...
            try
            {
                ...
                if (M.Text.Length > 0)
                {
                    if (vorhandeneKnotenlast.Lastwerte.Length < 3)
                    {
                        _ = MessageBox.Show("Momentenlast kann an diesem Knoten nicht aufgenommen werden", "neue Knotenlast");
                    }
                    else vorhandeneKnotenlast.Lastwerte[2] = double.Parse(M.Text);
                }
            }
        ...
        else
        {
            ...
            else { _ = MessageBox.Show("Lastknoten nicht definiert", "neue Knotenlast"); return; }
            // test, ob Lastknoten im Modell (Dictionary) vorhanden ist
            if (_modell.Knoten.TryGetValue(knotenId, out var lastKnoten)) { }
            else { _ = MessageBox.Show("Lastknoten im Modell nicht gefunden", "neue Knotenlast"); return; }
            ...
            var knotenlast = lastKnoten.AnzahlKnotenfreiheitsgrade switch
            {
    ...
    {
        Close();
        StartFenster.StabwerkVisual.IsKnotenlast = false;
    }
